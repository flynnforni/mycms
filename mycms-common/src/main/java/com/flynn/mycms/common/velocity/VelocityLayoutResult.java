...
import com.flynn.mycms.common.base.CmsUrl;
...
public class VelocityLayoutResult extends VelocityResult {
	...
	private Map<String, CmsUrl> velocityUrl;
	/**
	...
	private void mergeUrl(Context context, Map<String, CmsUrl> map) {
		if (map != null) {
			for (Map.Entry<String, CmsUrl> stringCmsUrlEntry : map.entrySet()) {
				String key = stringCmsUrlEntry.getKey();
				CmsUrl org = stringCmsUrlEntry.getValue();
				CmsUrl value = org.clone();//解决多线程并发的问题。
				value.setCmsUrl(org); //原始的不拿出来配置。
	...
	public void setVelocityUrl(Map<String, CmsUrl> velocityUrl) {
